...
export async function fetchAutocomplete(query, limit = 6) {
  ...
  try {
    const { data: enhancedData, error: enhancedError } = await supabase.rpc(
      ...
      { search_term: query.trim(), limit_count: limit }
    );
...
// ═══════════════════════════════════════════════════════════════
// 🔍 PRIMARY MATERIAL SEARCH — calls analyze-material edge fn
// ═══════════════════════════════════════════════════════════════
export async function searchBioLens(query) {
  ...
  // Convert 0-1 score to display range, parse strings safely
  const toScore = (val, multiplier = 100) =>
    val != null ? Math.round(parseFloat(val) * multiplier) : null;

  // Lifecycle: map edge-fn field names + convert scores
  const lc = data.lifecycle;
  const lifecycle = lc && lc.available
    ? {
        composite_score: toScore(lc.composite_score, 100),
        recyclability: toScore(lc.recyclability_score, 10),
        compostability: toScore(lc.compostability_score, 10),
        microplastic_risk: toScore(lc.microplastic_risk_score, 10),
        landfill_persistence_years: _persistenceScoreToYears(lc.landfill_persistence_score),
        best_end_of_life: _formatEndOfLifePathway(lc.best_end_of_life_pathway),
      }
    : null;

  // Exposure: map edge-fn field names
  const ex = data.exposure;
  const exposure = ex && ex.available
    ? {
        exposure_tier: ex.exposure_tier,
        concern_dimensions: ex.highest_concern_dimension
          ? [ex.highest_concern_dimension]
          : [],
        evidence_basis: ex.consumer_summary || null,
        confidence_label: ex.evidence_quality || null,
      }
    : null;
  ...
  return {
    ...
    framework_tier: data.framework_tier,
    petroload_score: toScore(data.petroload_score, 100),
    risk_level: data.risk_level,
    risk_label: data.risk_label,
    risk_color: data.risk_color,
    overall_material_health_score: toScore(data.overall_material_health_score, 100),
    confidence_score: data.confidence_score,
    match_source: data.match_source,
    pesticide_risk_score: toScore(data.pesticide_risk_score, 10),
    petro_ag_dependency_score: toScore(data.petro_ag_dependency_score, 10),
    processing_chemical_risk_score: toScore(data.processing_chemical_risk_score, 10),
    explanation: data.explanation,
    alternatives: data.alternatives || [],
    lifecycle,
    exposure,
    origin_intelligence: null,
    has_fiberfoundry_products: data.has_fiberfoundry_products || false,
  };
}

function _persistenceScoreToYears(score) {
  ...
  if (s <= 0.55) return 25;
  if (s <= 0.70) return 75;
  if (s <= 0.85) return 200;
  if (s <= 0.95) return 400;
  return 600;
}

function _formatEndOfLifePathway(pathway) {
  if (!pathway) return null;
  const labels = {
    mechanical_recycle: 'Mechanical Recycling',
    chemical_recycle: 'Chemical Recycling',
    industrial_compost: 'Industrial Composting',
    home_compost: 'Home Composting',
    biodegradation: 'Biodegradation',
    downcycle: 'Downcycle',
    landfill: 'Landfill',
  };
  return labels[pathway] || pathway.replace(/_/g, ' ');
}
...
const CATEGORY_MATERIAL_PROFILES = {
  'household_cleaning': {
    primary_materials: [
      { material: 'HDPE Bottle', component: 'Container', likelihood: 0.70, petroload: 75, description: 'High-density polyethylene bottle - most common for household cleaners' },
      { material: 'PP Trigger Sprayer', component: 'Dispensing mechanism', likelihood: 0.85, petroload: 78, description: 'Polypropylene trigger assembly with metal spring components' },
      { material: 'BOPP Label', component: 'Product label', likelihood: 0.90, petroload: 82, description: 'Biaxially oriented polypropylene film label with adhesive backing' },
    ],
  ...
  'laundry_detergent': {
    primary_materials: [
      { material: 'HDPE Jug', component: 'Container', likelihood: 0.80, petroload: 75, description: 'Large high-density polyethylene container for liquid detergent' },
      { material: 'PP Cap', component: 'Closure/Measuring Cup', likelihood: 0.95, petroload: 78, description: 'Polypropylene cap that often doubles as a measuring cup' },
    ],
  ...
  'shampoo_conditioner': {
    primary_materials: [
      { material: 'HDPE Bottle', component: 'Container', likelihood: 0.65, petroload: 75, description: 'High-density polyethylene bottle with flip-top or pump dispenser' },
      { material: 'PP Cap', component: 'Flip Cap/Pump', likelihood: 0.90, petroload: 78, description: 'Polypropylene flip-top cap or pump mechanism' },
    ],
  ...
  'soft_drinks': {
    primary_materials: [
      { material: 'PET Bottle', component: 'Container', likelihood: 0.90, petroload: 90, description: 'Polyethylene terephthalate bottle for carbonated beverages' },
      { material: 'HDPE Cap', component: 'Closure', likelihood: 0.95, petroload: 75, description: 'High-density polyethylene screw cap with liner' },
      { material: 'PET Shrink Label', component: 'Label', likelihood: 0.80, petroload: 90, description: 'Heat-shrink PET film label with printed graphics' },
    ],
    confidence: 'high',
  ...
  'batteries': {
    primary_materials: [
      { material: 'Steel Casing', component: 'Outer Shell', likelihood: 0.90, petroload: 45, description: 'Steel cylindrical or rectangular casing for protection' },
      { material: 'Alkaline Chemistry', component: 'Internal Components', likelihood: 0.80, petroload: 35, description: 'Zinc anode, manganese dioxide cathode, potassium hydroxide electrolyte' },
      { material: 'Paperboard Packaging', component: 'Retail Packaging', likelihood: 0.95, petroload: 25, description: 'Cardboard blister pack or box for retail display' },
    ],
    confidence: 'high',
  ...
  'electronics': {
    primary_materials: [
      { material: 'ABS Plastic', component: 'Housing/Casing', likelihood: 0.60, petroload: 88, description: 'Acrylonitrile butadiene styrene - tough plastic for electronics housings' },
      { material: 'Polycarbonate', component: 'Clear Components', likelihood: 0.30, petroload: 90, description: 'Impact-resistant clear plastic for screens and lenses' },
      { material: 'FR-4 PCB', component: 'Circuit Boards', likelihood: 0.95, petroload: 65, description: 'Fiberglass-reinforced epoxy laminate circuit board substrate' },
    ],
    confidence: 'medium',
  ...
  'cereal': {
    primary_materials: [
      { material: 'Paperboard Box', component: 'Outer Packaging', likelihood: 0.95, petroload: 30, description: 'Recycled paperboard with printed graphics and coatings' },
      { material: 'LDPE Liner', component: 'Inner Bag', likelihood: 0.85, petroload: 82, description: 'Low-density polyethylene bag to maintain freshness' },
    ],
    confidence: 'high',
  ...
  'bread': {
    primary_materials: [
      { material: 'LDPE Bag', component: 'Primary Packaging', likelihood: 0.90, petroload: 82, description: 'Low-density polyethylene bag with perforations for freshness' },
      { material: 'Plastic Coated Wire Tie', component: 'Closure', likelihood: 0.80, petroload: 70, description: 'Wire twist tie with plastic coating' },
    ],
    confidence: 'high',
...
function detectProductCategory(productName, apiCategory, company) {
  ...
  if (name.includes('cereal') || category.includes('breakfast cereal') || category.includes('cereal')) {
    return 'cereal';
  }
  if (name.includes('bread') || category.includes('bread') || category.includes('bakery')) {
    return 'bread';
  }
  if (companyName.includes('clorox') || companyName.includes('sc johnson')) return 'household_cleaning';
  if (companyName.includes('energizer') || companyName.includes('duracell')) return 'batteries';
...
function inferMaterialsFromCategory(categoryKey, productName, company) {
  ...
  if (!profile) {
    return {
      category: 'unknown', materials: [], estimatedPetroload: null,
      confidence: 'low', analysisType: 'none',
...
export async function lookupProductByBarcode(rawBarcode) {
  if (!rawBarcode) return { success: false, error: 'No barcode provided' };
  ...
  try {
    ...
    if (localResult.success) {
      return { ...localResult, source: 'BioLens Database (Curated)', companyInfo, confidenceLevel: 'high' };
    }
    const apiPromises = [
    ...
    if (successfulResults.length > 0) {
      ...
      if (!bestResult.product.brand && companyInfo) bestResult.product.brand = companyInfo.name;
      let finalMaterials = bestResult.materials;
      let inferredData = null;
      if (!finalMaterials || finalMaterials.length === 0) {
        const categoryKey = detectProductCategory(bestResult.product.name, bestResult.product.category, companyInfo);
      ...
      return { ...bestResult, companyInfo, materials: finalMaterials, inferredMaterials: inferredData, confidenceLevel, isInferred: !!inferredData };
    }
    if (companyInfo) {
      ...
      return {
        success: true, source: 'Company Prefix + Category Inference', isGeneric: true, companyInfo,
        product: { name: `${companyInfo.name} Product`, brand: companyInfo.name, category: companyInfo.sector, barcode: normalizedBarcode },
        materials, inferredMaterials: inferredData, confidenceLevel: 'low', isInferred: true,
        message: `Identified as a ${companyInfo.name} product. Material analysis based on typical ${companyInfo.sector} products.`,
      };
    }
    return {
      success: false, error: 'Product not found', barcode: normalizedBarcode,
      message: `Product with barcode ${normalizedBarcode} not found in any database.`,
      suggestions: ['Try scanning from a different angle or better lighting', 'Search for materials by product name instead', 'Ensure the barcode is clean and fully visible'],
    };
  } catch (error) {
    return { success: false, error: 'System error', companyInfo, message: 'Barcode lookup service temporarily unavailable. Please try again.' };
  }
}

async function lookupOpenBeautyFacts(barcode) {
  try {
    const response = await fetch(`https://world.openbeautyfacts.org/api/v2/product/${barcode}.json`, { headers: { 'User-Agent': 'BioLens-MaterialIntelligence/1.0 (Contact: support@biolens.app)' } });
    if (!response.ok) return { success: false };
    const data = await response.json();
    if (data.status === 0 || !data.product) return { success: false };
    const product = data.product;
    const materials = await extractMaterialsFromIngredients(product.ingredients_text_en || product.ingredients_text, product.packaging_text);
    return {
      success: true, source: 'Open Beauty Facts',
      product: { name: product.product_name || product.product_name_en || 'Unknown Beauty Product', brand: extractBrandName(product.brands), category: extractCategoryName(product.categories_tags), barcode, image: product.image_url || product.image_front_url, ingredients: product.ingredients_text_en || product.ingredients_text, packaging: product.packaging_text },
      materials,
      dataQuality: { completeness: calculateDataCompleteness(product), lastUpdated: product.last_modified_t ? new Date(product.last_modified_t * 1000).toLocaleDateString() : null },
    };
...
async function lookupOpenFoodFacts(barcode) {
  try {
    const response = await fetch(`https://world.openfoodfacts.org/api/v2/product/${barcode}.json`, { headers: { 'User-Agent': 'BioLens-MaterialIntelligence/1.0 (Contact: support@biolens.app)' } });
    if (!response.ok) return { success: false };
    const data = await response.json();
    if (data.status === 0 || !data.product) return { success: false };
    const product = data.product;
    const materials = await extractMaterialsFromIngredients(product.ingredients_text_en || product.ingredients_text, product.packaging_text);
    return {
      success: true, source: 'Open Food Facts',
      product: { name: product.product_name || product.product_name_en || 'Unknown Food Product', brand: extractBrandName(product.brands), category: extractCategoryName(product.categories_tags), barcode, image: product.image_url || product.image_front_url, ingredients: product.ingredients_text_en || product.ingredients_text, packaging: product.packaging_text, nutritionGrade: product.nutrition_grades, ecoScore: product.ecoscore_grade },
      materials,
      dataQuality: { completeness: calculateDataCompleteness(product), lastUpdated: product.last_modified_t ? new Date(product.last_modified_t * 1000).toLocaleDateString() : null },
    };
  } catch { return { success: false }; }
...
async function lookupUPCItemDB(barcode) {
  try {
    ...
    if (data.code === "OK" && data.items?.length > 0) {
      ...
      return {
        success: true, source: 'UPC Item DB',
        product: { name: item.title || 'Unknown Product', brand: item.brand || null, category: item.category || null, barcode, description: item.description || null, image: item.images && item.images.length > 0 ? item.images[0] : null },
        materials: [],
        dataQuality: { completeness: 75, lastUpdated: null },
      };
    }
...
async function lookupOpenProductsFacts(barcode) {
  try {
    const response = await fetch(`https://world.openproductsfacts.org/api/v0/product/${barcode}.json`, { headers: { 'User-Agent': 'BioLens-MaterialIntelligence/1.0 (Contact: support@biolens.app)' } });
    if (!response.ok) return { success: false };
    const data = await response.json();
    if (data.status === 1 && data.product) {
      const product = data.product;
      return {
        success: true, source: 'Open Products Facts',
        product: { name: product.product_name || product.generic_name || 'Unknown Product', brand: extractBrandName(product.brands), category: extractCategoryName(product.categories_tags), barcode, image: product.image_url || null },
        materials: [],
        dataQuality: { completeness: 60, lastUpdated: product.last_modified_t ? new Date(product.last_modified_t * 1000).toLocaleDateString() : null },
      };
    }
    return { success: false };
  } catch { return { success: false }; }
...
async function lookupLocalDatabase(barcode) {
  try {
    const barcodeVariations = [barcode, barcode.padStart(12, '0'), barcode.padStart(13, '0'), barcode.replace(/^0+/, '') || '0'];
    const { data, error } = await supabase
      .from('product_barcodes')
      .select(`barcode, products ( id, product_name, brands (brand_name), categories (category_name) )`)
    ...
    const { data: materialData } = await supabase
      ...
      .select(`percentage, materials ( material_name, material_family, petroload_score, biodegradability_score, toxicity_score, consumer_facing_summary )`)
      .eq('product_id', product.products.id);
    const materials = (materialData || []).map(m => ({
      name: m.materials.material_name, family: m.materials.material_family, percentage: m.percentage,
      petroloadScore: m.materials.petroload_score, biodegradabilityScore: m.materials.biodegradability_score,
      toxicityScore: m.materials.toxicity_score, summary: m.materials.consumer_facing_summary, source: 'curated', confidence: 1.0,
    }));
    return {
      success: true,
      product: { name: product.products.product_name, brand: product.products.brands?.brand_name, category: product.products.categories?.category_name, barcode },
      materials,
    };
  } catch { return { success: false }; }
}
...
async function extractMaterialsFromIngredients(ingredientsText, packagingText) {
  ...
  const analysisText = [ingredientsText, packagingText].filter(Boolean).join(' ').toLowerCase();
  ...
  for (const pattern of materialPatterns) {
    ...
    if (found && !detectedMaterials.has(pattern.dbMatch)) {
      ...
      if (matchedMaterials && matchedMaterials.length > 0) {
        ...
        materials.push({ name: material.material_name, family: material.material_family, petroloadScore: material.petroload_score, biodegradabilityScore: material.biodegradability_score, toxicityScore: material.toxicity_score, summary: material.consumer_facing_summary, source: 'detected', confidence: 0.8 });
      }
    }
  }
  return materials;
...
function calculateDataCompleteness(product) {
  const fields = [product.product_name, product.brands, product.categories_tags?.length, product.ingredients_text, product.packaging_text, product.image_url];
...
function convertInferredMaterialsToStandardFormat(inferredData) {
  ...
  return inferredData.materials.map(material => ({
    name: material.material, component: material.component, family: 'Inferred',
    petroloadScore: material.petroload, likelihood: material.likelihood,
    summary: material.description, source: 'inferred', confidence: inferredData.confidence,
  }));
}

function calculateConfidenceLevel(productResult, companyInfo, inferredData) {
  if (productResult.source === 'BioLens Database (Curated)' || (productResult.materials && productResult.materials.length > 0)) return 'high';
  if (productResult.product && productResult.product.category && inferredData) return 'medium';
  return 'low';
}
...
export async function searchMaterialsEnhanced(query, limit = 12) {
  if (!query || query.trim() === '') return getFeaturedMaterials(limit);
  try {
    const { data, error } = await supabase.rpc('search_materials_with_alternatives', { search_term: query.trim(), limit_count: limit });
    if (error) { console.error('Enhanced search error:', error); return []; }
    return data || [];
  } catch { return []; }
}
...
export async function getMaterialWithAlternatives(materialId) {
  ...
  try {
    const { data, error } = await supabase.rpc('get_material_with_alternatives', { material_uuid: materialId });
    if (error) { console.error('Material details error:', error); return null; }
    return data;
  } catch { return null; }
}
...
export async function getFeaturedMaterials(limit = 6) {
  try {
    const { data, error } = await supabase
      .from('materials')
      .select('id, material_name, material_family, petroload_score, biodegradability_score, toxicity_score, consumer_facing_summary, risk_level')
      .gt('petroload_score', 0.8)
      .not('consumer_facing_summary', 'is', null)
      .order('petroload_score', { ascending: false })
      .limit(limit);
    if (error) throw error;
    return (data || []).map(material => ({ ...material, alternatives_count: 0, match_type: 'featured' }));
  } catch { return []; }
}

export function getImpactLevel(score) {
  if (score == null) return { text: 'Unknown', level: 'unknown', class: 'bg-gray-500 text-white', color: '#86868B' };
  if (score >= 0.8) return { text: 'High Impact', level: 'high', class: 'bg-red-500 text-white', color: '#EF4444' };
  if (score >= 0.5) return { text: 'Medium Impact', level: 'medium', class: 'bg-yellow-500 text-white', color: '#F59E0B' };
...
export async function fetchAlternativeProducts(query, limit = 6) {
  const { data, error } = await supabase.rpc("get_best_alternative_products_for_query", { user_query: query, p_limit: limit });
  if (error) { console.error("Alt products error:", error); return []; }
  if (!data) return [];
  return data.map((p) => ({
    productId: p.product_id, title: p.product_title, brand: p.brand_name, category: p.category_name,
    price: p.display_price, transparencyScore: p.transparency_score, transparencyGrade: p.transparency_grade,
    petroloadScore: pct(p.petroload_score), trustScore: p.trust_score, purchaseUrl: p.purchase_url,
    alternativeMaterial: p.alternative_material_name, replacementReason: p.replacement_reason,
...
export function saveScanToHistory(query, result) {
  ...
  const entry = {
    ...
    materialName: result.material_name, materialClass: result.material_class,
    riskLevel: result.risk_level, petroloadScore: result.petroload_score,
    healthScore: result.overall_material_health_score, timestamp: new Date().toISOString(),
  };
