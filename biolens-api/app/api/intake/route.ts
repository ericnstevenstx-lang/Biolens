...
function hash(v: string) { return createHash("sha256").update(v).digest("hex").slice(0,16); }
...
function extractAsin(url: string): string | undefined {
  for (const p of [/\/dp\/([A-Z0-9]{10})/i, /\/gp\/product\/([A-Z0-9]{10})/i, /[?&]asin=([A-Z0-9]{10})/i]) {
    const m = url.match(p); if (m?.[1]) return m[1].toUpperCase();
  }
}
...
function parseAmazonHtml(html: string, asin: string, rawUrl: string): ExtractedProduct {
  const title = extractFromHtml(html, /<span[^>]+id="productTitle"[^>]*>\s*([\s\S]*?)\s*<\/span>/i);
  const brand = extractFromHtml(html, /id="bylineInfo"[^>]*>.*?<a[^>]*>([^<]+)<\/a>/is);
  ...
  const soldBy = extractFromHtml(html, /Sold by[^:]*:\s*<\/[^>]+>\s*<a[^>]*>([^<]+)<\/a>/i);
  const bullets: string[] = [];
  const br = /<span class="a-list-item">\s*([\s\S]*?)\s*<\/span>/gi;
  let m: RegExpExecArray | null;
  while ((m = br.exec(html)) !== null && bullets.length < 8) {
    const t = m[1].replace(/<[^>]+>/g, "").trim();
    if (t.length > 5 && t.length < 300) bullets.push(t);
  }
  return { asin, title, brand, imageUrl, countryOfOrigin, shipsFrom, soldBy,
    bullets: bullets.length ? bullets : undefined, rawPayload: { asin, url: rawUrl, scraped: true } };
}import { NextRequest, NextResponse } from "next/server";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import { createHash } from "crypto";
...
async function fetchAmazonProduct(asin: string, rawUrl: string): Promise<ExtractedProduct> {
  try {
    const res = await fetch(`https://www.amazon.com/dp/${asin}`,
      { headers: { "User-Agent": "Mozilla/5.0 (compatible; BioLens/1.0)" }, signal: AbortSignal.timeout(8000) });
...
async function fetchGenericUrl(url: string): Promise<ExtractedProduct> {
  try {
    ...
    return {
      title: x(/<meta[^>]+property="og:title"[^>]+content="([^"]+)"/i) ?? x(/<title>([^<]+)</title>/i),
      imageUrl: x(/<meta[^>]+property="og:image"[^>]+content="([^"]+)"/i),
      description: x(/<meta[^>]+(?:name="description"|property="og:description")[^>]+content="([^"]{10,300})"/i),
...
function buildOriginSignal(e: ExtractedProduct): OriginSignal {
  ...
  return {
    manufacturing_country: e.countryOfOrigin, ships_from_country: e.shipsFrom,
    seller_name: e.soldBy, manufacturer_name: e.manufacturer,
    ...
    disclosure_level: dl, confidence: dl==="full"?0.85:dl==="partial"?0.5:0.1,
    is_origin_disclosed: !!(e.countryOfOrigin||e.shipsFrom),
    is_third_party_seller: isTP, is_marketplace_listing: isTP,
  };
}
...
function generateFlags(s: OriginSignal): OriginFlag[] {
  ...
  if(["china","vietnam","bangladesh","cambodia","indonesia"].some(c=>ol.includes(c)))
    f.push({ flag_code:"IMPORT_DEPENDENCY", flag_label:`Sourced from import-dependent region: ${s.manufacturing_country??s.ships_from_country}`, severity:"info" });
  return f;
}
...
async function upsertProduct(t: InputType, v: string, e: ExtractedProduct, ck: string): Promise<string> {
  ...
  const { data, error } = await sb.from("products").upsert({
    canonical_key: ck, product_title: e.title??v, normalized_title: normalizeTitle(e.title??v),
    manufacturer_name: e.manufacturer??e.brand??null, country_of_origin: e.countryOfOrigin??null,
    source_type: t, source_id: e.asin??(t==="barcode"?v:null), image_url: e.imageUrl??null, ...gf,
  }, { onConflict:"canonical_key" }).select("id").single();
  if (error||!data) throw new Error(`Product upsert failed: ${error?.message}`);
  return data.id as string;
}

async function insertProductSource(pid: string, t: InputType, v: string, e: ExtractedProduct): Promise<void> {
  const sb = await createSupabaseServerClient();
  const su = t==="amazon"?`https://www.amazon.com/dp/${e.asin}`:t==="url"?v:null;
  await sb.from("product_sources").insert({
    product_id: pid, source_type: t, source_url: su,
